...
import { tenses, TTenses } from "./types";
...
const ConjugationLearnTab = ({ verbs }: Props) => {
    const [ enabledTenses, setEnabledTenses ] = useState<TTenses[]>(["INDICATIVE_PRESENT", "INDICATIVE_PRETERITE", "INDICATIVE_FUTURE"]);
        ...
        {tenses.map((tense, index) => {
            return <div key={`${tense}-${index}`}>
                <input
                    ...
                    id={tense} name={tense} 
                    onChange={(e)=> { setEnabledTenses(prevState => prevState.includes(tense) ? prevState.filter(e => e !== tense) : [...prevState, tense]) }}
                />
